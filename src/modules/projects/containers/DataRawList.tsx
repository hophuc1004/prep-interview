...
const DataRawList = ({ arrData, updateStateModal, disabled, updateData }: Props) => {
        ...
        cell: ({ row }) => {
          return (
            <MenuActionBtn
              ...
              isDisabled={disabled}
              keyAction={`dataRaw`}
              title={'Push into Datasets'}
              onAction={(e) => {
                e.stopPropagation()
                updateData({ dataRawId: row?.original?.id })
                updateStateModal({ push_dataset: true })
                return
              }}
            />
          )
  ...
  const renderContent = () => {
    if (!flatData?.length && !isLoading && !isRefetching) {
      return (
        <NoProject
          type='noEmployee'
          emptyText={t('No data raw have been added yet.')}
